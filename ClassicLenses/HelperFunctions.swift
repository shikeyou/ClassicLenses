...
class HttpHelper {
    ...
    class func makeHttpRequest(requestUrl requestUrl: String, requestMethod: String, requestAddValues: [(String, String)]? = nil, requestBody: NSData? = nil, requestRemoveXsrfToken: Bool = false, requestDataHandler: (data: NSData?, response: NSURLResponse?, error: NSError?)->Void) {
    ...
    class func parseJsonDataAsDictionary(data: NSData) -> (NSDictionary, NSError?) {

        //parse json data
        do {
            let jsonData: AnyObject? = try NSJSONSerialization.JSONObjectWithData(data, options: NSJSONReadingOptions.AllowFragments)
            return (jsonData as! NSDictionary, nil)
        } catch let caught as NSError {
            //TODO: check that caught gives the expected message
            return (NSDictionary(), caught)
        }
    }
    
    //method that parses raw JSON data into an NSDictionary
    class func parseJsonDataAsArray(data: NSData) -> (NSArray, NSError?) {
        
        //parse json data
        do {
            let jsonData: AnyObject? = try NSJSONSerialization.JSONObjectWithData(data, options: NSJSONReadingOptions.AllowFragments)
            return (jsonData as! NSArray, nil)
        } catch let caught as NSError {
            //TODO: check that caught gives the expected message
            return (NSArray(), caught)
        }
    }
    ...
    class func openUrl(url: String, view: UIViewController) {
        ...
        var newUrl = url
        if url.substringToIndex(url.startIndex.advancedBy(4)) != "http" {
            newUrl = "http://\(url)"
        }
...
class FileHelper {
    ...
    class func saveImageAsJpeg(image: UIImage, filename: String) -> String {
        ...
        if let jid = jpegImageData {
            jid.writeToFile(filePath, atomically: true)
        }
    ...
    class func getDocumentPathForFile(filename: String) -> String {
        ...
        //TODO: simplify this
        return (NSURL(string: dirPath)?.URLByAppendingPathComponent(filename).absoluteString)!
    }
    
    class func createImageOnDisk(imageFileName: String?) -> String? {
        ...
        if let img = imageFileName {
            ...
            if let imageData = NSData(contentsOfURL: imageURL!) {
                ...
                FileHelper.saveImageAsJpeg(UIImage(data: imageData)!, filename: fileName!)
    ...
    class func retrieveImage(imageFileName: String?) -> UIImage? {
...
class UiHelper {
    ...
    class func showActivityIndicator(view view: UIView) {
    ...
    class func showAlertAsync(view view: UIViewController, title: String, msg: String) {
    ...
    class func showAlert(view view: UIViewController, title: String, msg: String) {
